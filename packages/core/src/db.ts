 ...
 * a real `UPDATE <table> SET <deletedAtColumn> = now() ...`. Runs against
 * `withoutPlugins()` so the soft-delete filter doesn't stop it from
 * updating a row that isn't deleted yet.
 */
export function softDeleteUpdate<DB, TB extends keyof DB & string>(
  ...
  return updateBuilder.set({ [deletedAtColumn]: new Date() });
}
