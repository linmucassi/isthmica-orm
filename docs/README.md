...
`softDeleteUpdate()` for writes, and a `withDeleted()` escape hatch). That's
it. Audit/CDC, tenant isolation, migrations, and partitioning are designed
(see [`roadmap.md`](./roadmap.md)) but not yet built. If a page below
describes something not in that list, it's marked 📋 Planned — treat it as
intent, not as something you can `import` today.
